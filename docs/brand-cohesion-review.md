...
- **Removals (breaking — next release is a major):** `BlurText`, `CountUp`, `SpotlightCard`, deprecated `PRIDE_COLORS`/`firePrideBurst` — verified unused in FiestaBoard `web/`. **Correction (post-v1.0.0):** the original removal pass also dropped `Checkbox` and the `PageHeader`/`PageLayout`/`PageToolbar` exports after grepping only `web/src` — but FiestaBoard's routes live in `web/app/`, which imports all four (`Page*` scaffolding in `root.tsx`/`home.tsx`/`pages._index.tsx` since 0.3.0, `Checkbox` on the login page). All four were restored in the next release; the `Page*` family (and its `--icon-g*`/`.page-title` plumbing) remains the preferred page scaffolding. Lesson: downstream-usage checks must grep all of `web/`, not just `web/src`.
